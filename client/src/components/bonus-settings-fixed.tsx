...
export function BonusSettings() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: BonusSettings) => {
      console.log("Enviando dados de bônus via POST:", data);
      
      // Convertendo strings vazias para false quando necessário
      const processedData = {
        signupBonus: {
          enabled: data.signupBonusEnabled === true,
          amount: data.signupBonusAmount,
          rollover: data.signupBonusRollover,
          expiration: data.signupBonusExpiration
        },
        firstDepositBonus: {
          enabled: data.firstDepositBonusEnabled === true,
      ...
      const res = await apiRequest("POST", "/api/admin/bonus-settings", processedData);
    ...
    onSuccess: (data) => {
      ...
      // Verifica configurações atuais para determinar status
      if (form.getValues("signupBonusEnabled")) {
        bonusesAtivados.push("Bônus de Cadastro");
      }
      if (form.getValues("firstDepositBonusEnabled")) {
      ...
      refetchSettings()
        .then(() => {
          console.log("Configurações atualizadas com sucesso após salvar");
        ...
        .catch(error => {
          console.error("Erro ao atualizar configurações após salvar:", error);
        });
    },
    onError: (error: Error) => {
      toast({
  ...
  const onSubmit = (values: BonusSettings) => {
    ...
    console.log("Submetendo dados de bônus:", values);
